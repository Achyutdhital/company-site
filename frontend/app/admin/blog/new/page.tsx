...
export default function NewPost(){
  ...
  async function handleSubmit(e:React.FormEvent){ e.preventDefault(); setError(''); const token = localStorage.getItem('admin_token'); if(!token) return router.push('/admin/login');
    const payload = { title, slug, category, excerpt, body };
    try{ const res = await fetch(`${process.env.NEXT_PUBLIC_API_BASE}/admin/blog/`, { method:'POST', headers: { 'Content-Type':'application/json', Authorization:`Bearer ${token}` }, body: JSON.stringify(payload) }); if(!res.ok){ setError('Create failed'); return; } router.push('/admin/blog'); }catch(e){ setError('Create failed'); }
  }
