...
export default function AdminProjects(){
  ...
  useEffect(()=>{
    ...
    fetch(`${process.env.NEXT_PUBLIC_API_BASE}/admin/portfolio/`, { headers: { Authorization: `Bearer ${token}` } })
  ...
  return (
    <div className="p-6">
      <div className="flex items-center justify-between mb-4">
        ...
        <button onClick={()=> router.push('/admin/projects/new')} className="px-3 py-1 bg-sky-600 text-white rounded">New Project</button>
      </div>
      {loading ? <div>Loading...</div> : (
        <div className="space-y-3">{items.map(it=> (
          <div key={it.id||it.slug} className="p-3 border rounded flex justify-between items-center">
            ...
            <div>
              <button onClick={()=> router.push(`/admin/projects/${it.slug}`)} className="px-2 py-1 bg-slate-200 rounded">Edit</button>
            </div>
          </div>))}</div>
      )}
    </div>
